...
-- Written as a DO block rather than a bare ALTER for two reasons: the existing
-- constraint's name was auto-generated by Postgres and is only *probably*
-- activities_type_check, and this way the whole migration can be re-run safely
-- if a later statement fails and you paste the file again.
do $$
declare
  v_name text;
begin
  -- Drop whichever CHECK currently governs activities.type, whatever it is called.
  for v_name in
    select con.conname
      from pg_constraint con
     where con.conrelid = 'public.activities'::regclass
       and con.contype  = 'c'
       and pg_get_constraintdef(con.oid) ilike '%status_change%'
  loop
...
drop policy if exists "api_keys: admin read own org" on public.api_keys;
